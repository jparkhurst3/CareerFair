...
# 1 "levelTwoImage.h" 1
# 22 "levelTwoImage.h"
extern const unsigned short levelTwoImageTiles[672];


extern const unsigned short levelTwoImageMap[1024];


extern const unsigned short levelTwoImagePal[256];
# 4 "levelTwo.c" 2
# 1 "spriteSheet.h" 1
# 21 "spriteSheet.h"
...
void loadBackground(){
    loadPalette(levelTwoImagePal);
    DMANow(3, levelTwoImageTiles, &((charblock *)0x6000000)[0], (1344/2));
    DMANow(3, levelTwoImageMap, &((screenblock *)0x6000000)[27], (2048/2));
}
...
void drawRecruiters() {
 ...
 for(int i=0; i<(numRecruiters-3); i++) {
  shadowOAM[5+i].attr0 = (0 << 13) | (0 << 14) | (0xFF & recruiters[i].row);
  shadowOAM[5+i].attr1 = (2 << 14) | (0x1FF & recruiters[i].col);
  if(recruiters[i].alive == 0) {
   recruiters[i].counter++;
   if(recruiters[i].counter%200 == 0) {
    shadowOAM[5+i].attr2 = (4)*32+(24);
   } else {
    shadowOAM[5+i].attr2 = (0)*32+(24);
   }
  }
 }
 ...
 for(int i=(numRecruiters-3); i<numRecruiters; i++) {
  ...
  if(recruiters[i].alive == 0) {
   if(recruiters[i].counter%200 == 0) {
    shadowOAM[5+i].attr2 = (4)*32+(28);
   } else {
    shadowOAM[5+i].attr2 = (0)*32+(28);
   }
  }
 }
}
...
void drawPlayer() {
 shadowOAM[0].attr0 = (0 << 14) | player.row;
 shadowOAM[0].attr1 = (2 << 14) | player.col;
 shadowOAM[0].attr2 = (player.currFrame*2)*32+(player.aniState*4);
...
void drawGameClock() {
 ...
 shadowOAM[1].attr0 = (0 << 13) | (1 << 14) | gameClock.row;
 shadowOAM[1].attr1 = (2 << 14) | gameClock.col;
 shadowOAM[1].attr2 = ((gameClock.type*2)+6)*32+(20);
}

void drawResumes(int numResumes) {
 if(drawAllResumesFirst == 0) {
  for(int i=0; i<3; i++) {
   shadowOAM[2+i].attr0 = (0 << 13) | (0 << 14) | resumes[i].row;
   shadowOAM[2+i].attr1 = (1 << 14) | resumes[i].col;
   shadowOAM[2+i].attr2 = (4)*32+(20);
  }
  drawAllResumesFirst = 1;
 } else {
  for(int j=0; j<numResumes; j++) {
   shadowOAM[2+j].attr0 = (0 << 13) | (0 << 14) | resumes[j].row;
   shadowOAM[2+j].attr1 = (1 << 14) | resumes[j].col;
   shadowOAM[2+j].attr2 = (4)*32+(22);
  }
 }
}

void drawBill() {
 shadowOAM[19].attr0 = (0 << 13) | (0 << 14) | (0xFF & bill.row);
 shadowOAM[19].attr1 = (2 << 14) | (0x1FF & bill.col);
 shadowOAM[19].attr2 = (8)*32+(28);
}
